...
export default function Products() {
  ...
  const handleHeartClick = (id) => {
    ...
    let currentUserAddedIndex=likedData.findIndex((element)=> element.user?.includes(currentUserEmail) && element.id ==id) //checking whether the product is already added and logged in user has already added that product or not
    console.log({likedDataIndex,currentUserAddedIndex});
    if (likedDataIndex === -1   && currentUserEmail ) {
      console.log("if");
      likedData.push({ ...data[dataIndex], liked: true, user:[currentUserEmail] });
      data[dataIndex] = { ...data[dataIndex], liked: true };
      console.log(data);
      alert("Added to Wishlist");
    }else if(currentUserAddedIndex ==-1 && currentUserEmail){
      data[dataIndex] = { ...data[dataIndex], liked: true };
      likedData[likedDataIndex]={...likedData[likedDataIndex],user:[ ...likedData[likedDataIndex].user,currentUserEmail]}
      alert("Added to Wishlist");
    }
      ...
      likedData[likedDataIndex]={...likedData[likedDataIndex],user:likedData[currentUserAddedIndex].user.filter((element)=> element !=currentUserEmail)}; // [4,7,8]-> [4,8]
  ...
  return (
    <div className="productMain">
      ...
      <div className="productSubMain">
        <div className="productCard">{items.map((item, i) => (
          <Card
            actions={[
              item.liked ? (
                <HeartFilled
